  ...
  return (
    <div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        {/* Recent Attempts — Quiz / Paper tabs */}
        <div className="card p-5">
          <div className="flex items-center justify-between mb-4">
            <h2 className="font-semibold text-gray-900 flex items-center gap-2">
                ...
                {recentQuizAttempts.map(a => (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div className="min-w-0 flex-1 mr-3">
                      <p className="text-sm font-medium text-gray-800 truncate">{a.student_profiles?.full_name || 'Guest'}</p>
                      <p className="text-xs text-gray-400 truncate">{a.quizzes?.units?.title || 'Unknown Unit'}</p>
                    </div>
            ...
            recentPaperAttempts.length === 0 ? (
              ...
              <div className="space-y-2">
                {recentPaperAttempts.map(a => (
                  <div key={a.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                    <div className="min-w-0 flex-1 mr-3">
                      <p className="text-sm font-medium text-gray-800 truncate">{a.papers?.title || 'Unknown Paper'}</p>
                      <p className="text-xs text-gray-400">{a.papers?.subject} · Grade {a.papers?.grade}</p>
                    </div>
                    <div className="flex items-center gap-2 shrink-0">
