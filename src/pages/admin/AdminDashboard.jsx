  ...
  useEffect(() => {
    const safe = async (query) => {
      try {
        ...
        if (res.error) { console.error('Supabase:', res.error.message); return { count:0, data:[] } }
        return res
      } catch(err) { console.error('Error:', err); return { count:0, data:[] } }
    }
    ...
    Promise.all([
      ...
      // Recent quiz attempts — join student profile + unit title
      safe(supabaseAdmin.from('quiz_attempts')
        .select(`id, score, max_score, passed, submitted_at, language,
          student_profiles!quiz_attempts_student_id_fkey(full_name, grade, school_name),
          quizzes!quiz_attempts_quiz_id_fkey(
            units!quizzes_unit_id_fkey(title,
              chapters!units_chapter_id_fkey(subjects!chapters_subject_id_fkey(name))
            )
          )`)
        .not('submitted_at','is',null)
        .order('submitted_at', { ascending:false }).limit(8)),
      // Recent paper attempts — join student + paper info
      safe(supabaseAdmin.from('paper_attempts')
        .select(`id, score, max_score, passed, submitted_at, language, user_id,
    ...
    <div>
      ...
      <div className="flex items-center gap-3 mb-6">
        ...
        <div>
          ...
          <p className="text-gray-500 text-sm">RevWise.lk — Content & Student Management</p>
        </div>
      </div>
                ...
                <div key={s.id} className="flex items-center justify-between py-2 border-b border-gray-50 last:border-0">
                  <div className="flex items-center gap-2.5 min-w-0 flex-1 mr-2">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-gray-400 truncate">{s.school_name || 'No school'}</p>
                    </div>
                  </div>
            ...
            recentQuizAttempts.length === 0 ? (
              ...
              <div className="space-y-0 divide-y divide-gray-50">
                {recentQuizAttempts.map(a => {
                  const student = a.student_profiles
                  const unit = a.quizzes?.units
                  const subject = unit?.chapters?.subjects?.name
                  return (
                    <div key={a.id} className="py-2.5">
                      <div className="flex items-start justify-between gap-2">
                        <div className="min-w-0 flex-1">
                          <p className="text-sm font-medium text-gray-800 truncate">
                            {student?.full_name || 'Guest'}
                            {student?.grade && <span className="ml-1 text-xs text-gray-400">Gr.{student.grade}</span>}
                          </p>
                          <p className="text-xs text-gray-400 truncate">
                            {subject ? `${subject} · ` : ''}{unit?.title || 'Unknown Unit'}
                          </p>
                          <p className="text-xs text-gray-300">{fmtDate(a.submitted_at)}</p>
                        </div>
