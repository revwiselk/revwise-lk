...
export default function AdminPaperQuestions() {
  ...
  const saveOneQuestion = async (f, qId=null) => {
    ...
    else{const{data,error}=await supabaseAdmin.from('paper_questions').insert(payload).select().single();if(error)throw new Error(error.message);qId2=data.id}
    if(isM){
  ...
  return (
    <div>
      ...
      <PageHead crumb={`${section?.papers?.title} · ${section?.title}`} title="Questions"
