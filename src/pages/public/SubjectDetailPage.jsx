  ...
  return (
    <div ref={ref}>
      ...
      <div className="flex gap-2 overflow-x-auto pb-2 mb-4 -mx-1 px-1" style={{ scrollbarWidth: 'none' }}>
        {sections.map((s, i) => {
          const sst = IC[s.icon] || IC.book; const SIcon = sst.I
          return (
            <button key={i} onClick={() => setIdx(i)}
              className={clsx(
                'flex items-center gap-1.5 px-3 py-2 rounded-xl text-xs font-medium transition-all shrink-0 border-2',
                idx === i ? `${sst.a} border-transparent shadow-sm` : `${sst.idle} border-transparent`
              )}>
              <SIcon size={12} />{s.title || `Topic ${i + 1}`}
            </button>
          )
        })}
      </div>
      ...
      {/* Content card */}
      <div className="card overflow-hidden animate-fade-in" key={idx}>
        {/* Header */}
        <div className={clsx('flex items-center gap-3 px-5 py-3 border-b border-gray-100', idx === 0 ? 'bg-blue-50' : 'bg-gray-50')}>
          <div className={clsx('w-9 h-9 rounded-xl flex items-center justify-center shrink-0', st.a)}><Icon size={17} /></div>
        ...
        <div className="flex items-center justify-between px-5 pb-5 pt-2 border-t border-gray-100">
          ...
          <button onClick={goNext} className="btn-sm btn-blue gap-1.5">
            {isLast ? <><Zap size={13} /> Flashcards</> : <>Next <ChevronRight size={13} /></>}
          </button>
        </div>
      </div>
    </div>
...
function FlashcardDeck({ unitId, language, onComplete }) {
  ...
  if (!cards.length) return (
    <div className="flex flex-col items-center justify-center py-12 bg-gray-50 rounded-2xl border-2 border-dashed border-gray-200 text-center">
      ...
      <button onClick={() => onComplete?.()} className="btn-sm btn-blue mt-4 gap-1.5">Skip to Quiz <ChevronRight size={13} /></button>
    </div>
  )
  ...
  return (
    <div className="max-w-lg mx-auto">
      ...
      <div onClick={() => { setFlipped(!flipped); setHint(false) }}
        className={clsx('min-h-[180px] sm:min-h-[200px] rounded-2xl flex flex-col items-center justify-center p-6 text-center cursor-pointer select-none transition-all duration-200',
        ...
        {!flipped ? (
          <div className="w-full">
            ...
            {card.hint && !hint && <button onClick={e => { e.stopPropagation(); setHint(true) }} className="mt-4 text-xs text-blue-500 hover:underline">Hint</button>}
...
export default function SubjectDetailPage() {
  ...
  const { language } = useLangStore()  // global language
  ...
  // Show floating progress on tab change
  useEffect(() => {
  ...
  const fetchSubject = async () => {
    ...
    const [sRes, cRes] = await Promise.all([
      ...
      supabase.from('subjects')
        .select('*, subject_translations(language, name, description)')
        .eq('id', subjectId).single(),
      // Chapters with translations and units with their translations
      supabase.from('chapters')
        .select(`id, order_index, is_active,
          chapter_translations(language, title, description),
          units(id, order_index, is_active,
            unit_translations(language, title))`)
  ...
  const cur = unitContent.find(c => c.language === language)
    || unitContent.find(c => c.language === 'english')
    || unitContent[0]

  const activeUnitTitle = allUnits.find(u => u.id === activeUnit)
  const unitTitle = activeUnitTitle ? getUnitTitle(activeUnitTitle) : ''
  ...
  const SidebarContent = () => (
    <div className="flex flex-col h-full">
      <div className="bg-gradient-to-br from-blue-600 to-blue-800 p-4 text-white">
        <h1 className="font-bold text-lg mb-1">{subjectName}</h1>
        {subjectDesc && <p className="text-blue-200 text-sm line-clamp-2 mb-2">{subjectDesc}</p>}
        ...
        {chapters.map((ch, ci) => (
          <div key={ch.id} className="rounded-xl overflow-hidden border border-gray-100">
            <button onClick={() => setExpanded(p => ({ ...p, [ch.id]: !p[ch.id] }))}
              ...
              <div className="flex-1 min-w-0">
                <p className="text-sm font-semibold text-gray-900 truncate">{getChapterTitle(ch)}</p>
  ...
  return (
    <div className="max-w-7xl mx-auto px-4 sm:px-6 py-4 sm:py-6">
      ...
      <div className="flex gap-6">
        {/* Desktop sidebar */}
        <aside className="hidden lg:flex flex-col w-72 xl:w-80 shrink-0 rounded-2xl overflow-hidden border border-gray-200 shadow-sm"
                    ...
                    <div className="flex gap-3 mt-6 pt-5 border-t border-gray-100">
                      {unitIdx > 0 && (
                        <button onClick={() => goUnit(allUnits[unitIdx - 1])} className="btn-md btn-white gap-2 flex-1 justify-center min-w-0">
                          <ChevronLeft size={15} /><span className="truncate text-xs sm:text-sm">{getUnitTitle(allUnits[unitIdx - 1])}</span>
                        </button>
                      )}
                      {unitIdx < allUnits.length - 1 && (
                        <button onClick={() => { toggleDone(activeUnit); goUnit(allUnits[unitIdx + 1]) }} className="btn-md btn-blue gap-2 flex-1 justify-center min-w-0">
                          <span className="truncate text-xs sm:text-sm">{getUnitTitle(allUnits[unitIdx + 1])}</span><ChevronRight size={15} />
                        </button>
                      )}
                    </div>
                  )}
